...
def fig6_real_rejection_curves(real_data: dict, out_dir: str):
    ...
    set_style()
    diseases = [
        ('MS', 'MS Cohort  (τ=0.11, Severe Imbalance)',  DISEASE_MARKERS['MS']['color']),
        ('PD', 'PD Cohort  (τ=0.29, Moderate Imbalance)', DISEASE_MARKERS['PD']['color']),
...
# ─────────────────────────────────────────────
# FIG 8 — ccAUGRC REAL-WORLD TABLE
# ─────────────────────────────────────────────
...
def main(scalars_path: str, curves_path: str, real_dir: str, out_dir: str):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--real_dir', default='',
                        help='Directory with real-world result CSVs')
    parser.add_argument('--out_dir',  required=True)
    args = parser.parse_args()

    main(args.scalars, args.curves, args.real_dir, args.out_dir)